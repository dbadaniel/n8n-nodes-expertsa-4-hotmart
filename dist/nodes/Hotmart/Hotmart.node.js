            ...
            try {
                ...
                if (resource === 'sales') {
                    if (operation === 'getAll') {
                        ...
                        const transactionCode = this.getNodeParameter('transactionCode', i);
                ...
                if (resource === 'subscriptions') {
                    if (operation === 'getAll') {
                        ...
                        method = 'POST';
                        const subscriberCodesStr = this.getNodeParameter('subscriberCodes', i, '');
                        const sendMail = this.getNodeParameter('sendMail', i, true);
                        ...
                        method = 'POST';
                        const charge = this.getNodeParameter('charge', i, false);
                        qs.charge = charge;
                    }
                ...
                if (resource === 'products') {
                    if (operation === 'getAll') {
                        ...
                        const productUcode = this.getNodeParameter('productUcode', i);
                ...
                if (resource === 'members') {
                    ...
                    if (operation === 'getStudents' || operation === 'getStudentsProgress') {
                        ...
                        const productId = this.getNodeParameter('productId', i, 0);
                        if (productId) {
                            qs.product_id = productId;
                        }
                    }
                    else if (operation === 'getStudentProgress') {
                        const progressMode = this.getNodeParameter('progressMode', i, 'summary');
                        const filters = this.getNodeParameter('filters', i, {});
                        if (progressMode === 'summary') {
                            endpoint = '/club/api/v1/users';
                            if (filters.email) {
                                qs.email = filters.email.trim().replace(/["']/g, '');
                            }
                            if (filters.name) {
                            ...
                            let userId = (filters.userId || '').trim().replace(/["']/g, '');
                            ...
                            if (searchEmail) {
                                ...
                                do {
                                    ...
                                    const foundUser = (_a = userLookup === null || userLookup === void 0 ? void 0 : userLookup.items) === null || _a === void 0 ? void 0 : _a.find((u) => String(u.email || '').trim().toLowerCase() === searchEmail);
                                    if (foundUser && (foundUser.user_id || foundUser.id)) {
                                        foundUserId = String(foundUser.user_id || foundUser.id);
                                        break;
                                    }
                                    pageToken = userLookup === null || userLookup === void 0 ? void 0 : userLookup.page_token;
                                } while (pageToken);
                                if (foundUserId) {
                                    userId = foundUserId;
                                }
                ...
                if (response.items && Array.isArray(response.items)) {
                    const isProgressSummary = operation === 'getStudentsProgress' ||
                    ...
                    let itemsToProcess = isProgressSummary
                        ? response.items.map((student) => {
                            ...
                            return {
                                ...
                                access_count: (_a = student.access_count) !== null && _a !== void 0 ? _a : null,
                            };
                        })
                        : response.items;
                    if (isProgressSummary) {
                        ...
                        if (filters === null || filters === void 0 ? void 0 : filters.email) {
                            ...
                            itemsToProcess = itemsToProcess.filter((s) => String(s.email || '').trim().toLowerCase() === targetEmail);
                        }
                        if (filters === null || filters === void 0 ? void 0 : filters.userId) {
                            const targetId = String(filters.userId).trim();
                            itemsToProcess = itemsToProcess.filter((s) => String(s.user_id || s.id || '').trim() === targetId);
                        }
                    }
